...
# from meshtastic import BROADCAST_ADDR, export_config, traverseConfig
...
class Node:
    ...
    def node_configure(self, filename, *kwargs, **getNode_kwargs):
        with open(filename, encoding="utf8") as file:
            configuration = yaml.safe_load(filename)
            closeNow = True

            self.getNode(kwargs.dest, False, **getNode_kwargs).beginSettingsTransaction()
            ...
            if "owner" in configuration:
                ...
                waitForAckNak = True
                self.getNode(kwargs.dest, False, **getNode_kwargs).setOwner(configuration["owner"])
                time.sleep(0.5)
            ...
            if "owner_short" in configuration:
                ...
                waitForAckNak = True
                self.getNode(args.dest, False, **getNode_kwargs).setOwner(
            ...
            if "channel_url" in configuration:
                ...
                self.getNode(args.dest, **getNode_kwargs).setURL(configuration["channel_url"])
                time.sleep(0.5)

            if "channelUrl" in configuration:
                print("Setting channel url to", configuration["channelUrl"])
                self.getNode(args.dest, **getNode_kwargs).setURL(configuration["channelUrl"])
                time.sleep(0.5)

            if "canned_messages" in configuration:
                print("Setting canned message messages to", configuration["canned_messages"])
                self.getNode(args.dest, **getNode_kwargs).set_canned_message(configuration["canned_messages"])
                time.sleep(0.5)

            if "ringtone" in configuration:
                print("Setting ringtone to", configuration["ringtone"])
                self.getNode(args.dest, **getNode_kwargs).set_ringtone(configuration["ringtone"])
                time.sleep(0.5)
            ...
            if "location" in configuration:
                ...
                lat = 0.0
                lon = 0.0
                localConfig = self.localNode.localConfig
                ...
                self.localNode.setFixedPosition(lat, lon, alt)
                time.sleep(0.5)

            if "config" in configuration:
                localConfig = self.getNode(args.dest, **getNode_kwargs).localConfig
                for section in configuration["config"]:
                    traverseConfig(
                        section, configuration["config"][section], localConfig
                    )
                    self.getNode(args.dest, **getNode_kwargs).writeConfig(
                        camel_to_snake(section)
                    )
                    time.sleep(0.5)

            if "module_config" in configuration:
                moduleConfig = self.getNode(args.dest, **getNode_kwargs).moduleConfig
            ...
            self.getNode(args.dest, False, **getNode_kwargs).commitSettingsTransaction()
            print("Writing modified configuration to device")

    def node_export_config(self, interface, args):
